...
public class DelayedWorkTracker {
    ...
    public void schedule(final WorkSpec workSpec) {
        Runnable remove = this.mRunnables.remove(workSpec.f80id);
        if (remove != null) {
            this.mRunnableScheduler.cancel(remove);
        }
        C06721 r0 = new Runnable() {
            public void run() {
                Logger.get().debug(DelayedWorkTracker.TAG, String.format("Scheduling work %s", new Object[]{workSpec.f80id}), new Throwable[0]);
                DelayedWorkTracker.this.mGreedyScheduler.schedule(workSpec);
            }
        };
        this.mRunnables.put(workSpec.f80id, r0);
